...
<div style={{textAlign: 'right'}}>版本 1.0.0</div>

本文档规定了 JSpecify nullness annotation 的语义。

> 本文档主要对分析工具的作者有用。一些非常高级的用户可能会觉得有趣，但对于其他人来说，这是一个非常糟糕的入门介绍；请参阅我们的**[从这里开始](/docs/start-here)页面**。

## 规范性和非规范性章节

本文档包含一些非规范性注释，以强调要点或预期可能的问题。这些注释以块引用的形式呈现。

> 这是一个非规范性注释的示例。

本文档还链接到其他文档。这些文档是非规范性的，除非我们链接到 Java 语言规范以遵循其规则。

> ### "nullable" 一词
>
> 在本文档中，我们尽量避免提及类型"是 nullable"。相反，我们定义了四种可以对任何给定类型使用提出的"它是否 nullable？"问题：
>
> 1.  该类型使用的[augmented type]是什么？
> 2.  我是否必须处理 `null` 从中出现的情况？
> 3.  我是否必须防止 `null` 进入其中？
> 4.  对于 nullness，此类型是否是彼类型的子类型？
>
> ### 本规范的范围
>
> 本规范不涉及工具*何时*必须应用规范的任何部分。例如，它没有说明工具何时必须检查[subtyping]关系是否成立。
>
> 我们预期工具通常会在与它们（或 `javac`）已经应用 Java 语言规范相应部分相同的情况下应用本规范的每个部分。例如，如果代码包含参数化类型 `List<@Nullable Foo>`，我们预期工具会检查 `@Nullable Foo` 是 `List` 类型参数边界的子类型。
>
> 然而，这取决于工具作者，他们可能有理由采取不同的方法。例如：
>
> -   Java [施加了一些对于健全性并非必要的限制][#49]，而且它[在至少一种可能导致运行时错误的方式上是宽松的][#65]。
>
> -   JSpecify annotation 甚至可以被非"nullness checker"的工具使用。例如，列出 API 成员的工具可以显示 API 中每种类型的 nullness，而不检查这些类型是否"正确"。
>
> -   即使工具是"nullness checker"，它也可能是为另一种语言（如 Kotlin）编写的，有自己的类型检查执行规则。或者该工具可能针对未来版本的 Java，其语言特性不在本规范版本覆盖范围内。
>
> 另请注意，本规范仅涵盖*来自 JSpecify annotation*的 nullness 信息。工具可能有额外的信息来源。例如，工具可能识别额外的 annotation，或者工具可能省略 `UNSPECIFIED` 的概念并应用类型使用如 `Object` 始终为 non-nullable 的策略。
